...
def _match_plate_to_vehicle(frame: np.ndarray, vehicle_bbox: tuple[int, int, int, int],
                             all_plates: list[tuple[tuple[int, int, int, int], float]]) -> tuple[tuple[int, int, int, int] | None, np.ndarray | None]:
    """Match the best plate detection to a vehicle bbox.
    ...
    for (px1, py1, px2, py2), conf in all_plates:
        # Plate center must be inside (or near) the vehicle bbox
        plate_cx = (px1 + px2) // 2
        plate_cy = (py1 + py2) // 2
...
def detect(frame: np.ndarray, confidence_threshold: float = 0.5) -> list[DetectionResult]:
    ...
    Optimized for Indian roads: first tries Indian vehicle detector
    (auto-rickshaws, tempos, scooters, etc.), then falls back to
    COCO-based YOLOv8n with Indian-specific heuristics.
    ...
    # ── Try Indian vehicle detector first (detects autos, tempos, etc.) ──
        ...
        try:
            ...
            for result in india_results:
                ...
                for i in range(len(boxes)):
                    ...
                    # Map to standard Indian vehicle types
